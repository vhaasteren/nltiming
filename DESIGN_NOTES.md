...
`nltiming` owns the nonlinear-timing math, the timing backends, and both
likelihood frontends. It is a satellite of the timing ecosystem, not a
...
those frontends (noise) rather than re-homing noise math here. See
`TRANSFORMED_SPACE_FIT.md` for the interactive-fit seam this exposes to such
consumers.
...
tangent. Host residual parity is at the picosecond tier on curated fixtures;
see JUG's parity gates for production coverage.
...
frontends for the same pulsar and config. Both frontends column-normalize the
...
**Frontend consistency (hard-won)** — the Enterprise frontend must consume the
`TimingBinding` (backend, partition, space, design matrix), never re-query
`pulsar.timing_backend(...)` with partial kwargs: a backend rebuilt without
`subtract_tzr=False` evaluates a delay wrong by its own order of magnitude.
Both frontends evaluate the delay via `residual_delta_jax` when the backend
...
one binding) is the regression probe of record for this class of bug.
...
(landed on the `tempo2-dev` branch); `nltiming`'s jug backend imports them and
...
counts) currently ships host-side as
`metapulsar.discovery_compat.apply_discovery_compat_patches` and on a
`fix/make-uind-empty-basis` branch; the shim stays until it merges upstream.
...
**Vela / pyvela.** Vela.jl is now a first-class engine (`backends/vela.py`,
