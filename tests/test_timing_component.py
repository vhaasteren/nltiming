...
class _Pulsar:
    def __init__(self):
        ...
        # Offset = constant gauge column; F0 must not be collinear with it.
        ...
        design = np.column_stack(
            [
                ...
                np.array([0.2, 0.3, -0.1, 0.4, -0.2, 0.1, -0.2, 0.3]),
            ]
        ).astype(float)
        self._design = design
...
def _schur_fisher(pulsar, *, analytically_marginalize, variance):
    part = plan_for(pulsar, delta_flat=analytically_marginalize or ())
...
def test_component_config_only_build_and_with_engines():
    ...
    ntm = TimingSpec(
        ...
        inference=TimingInference.groups(delta_flat=["Offset", "F0"]),
    ...
    assert swapped.static_layer == ntm.static_layer
    assert swapped.inference == ntm.inference
...
def test_autodiff_design_matrix_method_feeds_whitening(pulsar):
    ...
    pulsar._jug_backend.residual_jacobian = lambda: -np.asarray(
        autodiff_matrix, dtype=float
    )
    ntm = TimingSpec(
        engines="jug",
    ...
    part = plan_for(pulsar, delta_flat=["Offset", "F0", "DM"])
...
def test_cheat_wls_prior_is_wide_uniform_box(pulsar):
    ...
    np.testing.assert_allclose(
        half_widths, ntm.coordinate_policy.nonlinear_scale * expected_stds, rtol=1e-6
    )
...
def test_autodiff_design_matrix_method_feeds_cheat_prior_widths(pulsar):
    ...
    block = ntm.for_pulsar(pulsar).priors
    part = plan_for(pulsar, sample_all=True)
    expected_fisher = schur_delta_wls(
        pulsar=pulsar,
        partition=part,
        variance=np.asarray(pulsar.toaerrs, dtype=float) ** 2,
...
def test_cheat_prior_box_clipped_to_physical_bounds():
    class _BoundedHost(_Pulsar):
        def __init__(self):
            super().__init__()
            self.fitpars = ("Offset", "ECC", "F1", "DM")
            self._ecc_ref = 1.0e-5
            # Reuse Offset + three independent columns; rename F0 → ECC.
            model = LinearModel.from_design(
                fitpars=self.fitpars,
                design=self._design,
                theta_exact={
                    "Offset": "0.0",
                    "ECC": repr(self._ecc_ref),
                    "F1": "1.0",
                    "DM": "5.0",
                },
            )
...
def test_autodiff_design_matrix_method_feeds_discovery_gp_basis(pulsar, monkeypatch):
    ...
    np.testing.assert_allclose(
        captured["basis"],
        normalized_basis(autodiff_matrix[:, [0, 1, 3]]),  # Offset, F0, DM
    )
...
def test_enterprise_signal_forwards_engines(pulsar):
    ...
    assert kwargs_seen["derivative_method"] == "analytic"
...
def test_timing_coord_distribution_log_prob_matches_logprior_coord(pulsar):
    ...
    cases = [(WhiteningConfig(), "x"), (None, "z")]
    for whitening, coord in cases:
        ntm = TimingSpec(
            engines="jug",
            whitening=whitening,
        ...
        assert np.isclose(got, expected, rtol=1e-6, atol=1e-8), (whitening, coord)


def test_timing_coord_x_and_z_add_no_extra_prior_factor(pulsar, monkeypatch):
    for whitening, coord in [(WhiteningConfig(), "x"), (None, "z")]:
        ntm = TimingSpec(
            engines="jug",
            whitening=whitening,
            inference=TimingInference.groups(delta_flat=["DM"]),
        ...
        assert calls["factor"] == [], (whitening, coord)


def _enterprise_pta(pulsar, *, whitening, analytically_marginalize):
    ...
    ntm = TimingSpec(
        engines="jug",
        whitening=whitening,
        inference=TimingInference.groups(delta_flat=analytically_marginalize),
        name="timing",
    )
...
@pytest.mark.parametrize(
    ("whitening", "analytically_marginalize"),
    [
        (None, ["DM"]),  # identity static layer (scalar z params)
        (WhiteningConfig(), ["F0", "DM"]),  # ndim=1: size-1 block stays a vector
        (WhiteningConfig(), ["DM"]),  # ndim=2, non-diagonal C from the design matrix
    ],
)
def test_enterprise_parameters_sample_and_evaluate_full_pta(
    pulsar, whitening, analytically_marginalize
):
    ...
    pta = _enterprise_pta(
        pulsar, whitening=whitening, analytically_marginalize=analytically_marginalize
    )
    x0 = np.hstack(
...
def test_whitening_vector_sample_and_ppf_describe_the_same_distribution(
    ...
    ntm = TimingSpec(
        engines="jug",
        whitening=WhiteningConfig(),
        inference=TimingInference.groups(delta_flat=["Offset", "DM"]),
        name="timing",
    )
    bound = ntm.enterprise_signal()(pulsar)
    (param,) = [p for p in bound.params if p.name.endswith("_timing_x")]
